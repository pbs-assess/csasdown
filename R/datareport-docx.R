...
datareport_docx <- function(...) {
  .csasdown_docx_base(
    reference_docx = "data-report-content.docx",
    link_citations = TRUE,
    template_dir = "data-report-docx",
    use_pandoc_highlight = FALSE,
    ...
  )
}

add_datareport_word_frontmatter <- function(index_fn, yaml_fn = "_bookdown.yml", verbose = FALSE, keep_files = FALSE) {
  if (verbose) cli_inform("Adding frontmatter to the Data Report using the officer package...")
  ...
  # Read the data report cover template and replace bookmarks
  cover_file <- if (french) "01-data-report-cover-french.docx" else "01-data-report-cover-english.docx"
  frontmatter <- officer::read_docx(system.file("data-report-docx", cover_file, package = "csasdown")) |>
  ...
  titlepage_file <- if (french) "03-data-report-titlepage-french.docx" else "03-data-report-titlepage-english.docx"
  titlepage <- officer::read_docx(system.file("data-report-docx", titlepage_file, package = "csasdown")) |>
  ...
  citation_prefix <- if (french) "Rapp. stat. can. sci. halieut. aquat. " else "Can. Data Rep. Fish. Aquat. Sci. "
  citation <- paste0(
    trimws(gsub("\n", " ", author_list)), " ",
    x$year, ". ",
    trimws(gsub("\n", " ", title)), ". ",
  ...
  colophon_file <- if (french) "04-data-report-colophon-french.docx" else "04-data-report-colophon-english.docx"
  colophon <- officer::read_docx(system.file("data-report-docx", colophon_file, package = "csasdown")) |>
    officer::body_replace_text_at_bkm("year", as.character(x$year)) |>
